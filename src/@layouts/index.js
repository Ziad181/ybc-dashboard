...
} from './composable/useDynamicVhCssProperty'
import {
  config
} from './config'
import {
...
export const createLayouts = userConfig => {
  ...
  const localStorageIsLang = localStorage.getItem(`${userConfig.app.title}-isLang`)
  ...
  config.app.isLang.value = localStorageIsLang ? localStorageIsLang : userConfig.app.isLang
  i18n.global.locale.value = localStorageIsLang ? localStorageIsLang : userConfig.app.isLang;
  ...
  return () => {
    ...
    _setAppLang(config.app.isLang.value)
    i18n.global.locale.value = config.app.isLang.value
  }
}
