...
const onSubmitForm = () => {
  refVForm.value?.validate().then(({ valid: isValid }) => {
    if (isValid) {
      store.updateBranch(formData.value).then((res) => {
        router.push({
...
onMounted(() => {
  store.loadBranchDetails(route.params.id).then(() => {
    formData.value.name_ar = store.getBranchDetails.name_ar;
    formData.value.name_en = store.getBranchDetails.name_en;
    formData.value.address = store.getBranchDetails.address;
    formData.value.geo_location = store.getBranchDetails.geo_location;
    formData.value.geo_location.lat =
      parseFloat(store.getBranchDetails.geo_location.lat) ?? null;
    formData.value.geo_location.lng =
      parseFloat(store.getBranchDetails.geo_location.lng) ?? null;
    centerAddress.value.lat =
      parseFloat(store.getBranchDetails.geo_location.lat) ?? null;
    centerAddress.value.lng =
      parseFloat(store.getBranchDetails.geo_location.lng) ?? null;
  });
});
</script>
