...
onMounted(() => {
  store.loadPartnerships(filteredData);
});
...
const changeStatus = (id, status) => {
  ...
  store.partnershipChangeStatus(data);
...
const deletePartnershipFun = (id, status) => {
  let data = {
    id: id,
  };
  store.deletePartnership(data);
...
<template>
  <section>
    <VRow>
      <VCol cols="12">
        <VCard :title="$t('nav.partnerships')">
          <!-- 👉 Filters -->
          <VCardText>
            <VRow>
              <!-- 👉 Search -->
              ...
              >
                ...
                <VBtn class="px-7" :to="{ name: 'partnerships-add' }">
                  {{ $t("common.add_partnership") }}
                </VBtn>
              </VCol>
              <!-- 👉 Select Status -->
          ...
          <VTable class="text-no-wrap">
            ...
            <tbody>
              <tr
                v-for="(item, index) in store.getPartnerships"
                :key="index"
              ...
              >
                ...
                <td class="pt-2 pb-3">
                  <div v-html="item.partnership_year"></div>
                </td>
                <td class="pt-2 pb-3">
            ...
            <!-- 👉 table footer  -->
            <tfoot v-show="!store.getPartnerships.length">
              <tr>
          ...
          <VCardText
            v-if="store.getTotalPartnerships.currentPage"
          ...
          >
            ...
            <VPagination
              v-model="store.getTotalPartnerships.currentPage"
              size="small"
              :total-visible="5"
              :length="store.getTotalPartnerships.totalPages"
