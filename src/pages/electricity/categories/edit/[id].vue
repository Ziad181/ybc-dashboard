...
const onSubmitForm = () => {
  refVForm.value?.validate().then(({ valid: isValid }) => {
    if (isValid) {
      ...
      store.updateCategory(formData.value).then((res) => {
        router.push({
...
onMounted(() => {
  store.loadCategoryDetails(route.params.id).then(() => {
    formData.value.name_ar = store.getCategoryDetails.name_ar;
    formData.value.name_en = store.getCategoryDetails.name_en;
    formData.value.sub_categories = store.getCategoryDetails.sub_categories;
  });
});
</script>
            ...
            <VRow>
              ...
              <VCol cols="12" class="d-flex flex-wrap gap-4 mt-5">
                ...
                <VBtn
                  ...
                  variant="tonal"
                  type="reset"
                  :to="{ name: 'electricity-categories' }"
                >
                  {{ $t("common.cancel") }}
                </VBtn>
              </VCol>
