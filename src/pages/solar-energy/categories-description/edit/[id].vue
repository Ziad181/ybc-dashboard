...
const onSubmitForm = () => {
  refVForm.value?.validate().then(({ valid: isValid }) => {
    if (isValid) {
      ...
      store.updateCategoryDescriptions(data).then((res) => {
        router.push({
          name: "solar-energy-categories-description",
        });
      });
    }
  });
...
onMounted(() => {
  store.loadCategoryDescriptionsDetails(route.params.id).then(() => {
    formData.value.section_name_ar =
      store.getCategoryDescriptionsDetails.section_name_ar;
    formData.value.section_name_en =
      store.getCategoryDescriptionsDetails.section_name_en;
    formData.value.description_ar =
      store.getCategoryDescriptionsDetails.description_ar;
    formData.value.description_en =
      store.getCategoryDescriptionsDetails.description_en;
    images.value = store.getCategoryDescriptionsDetails.image
      ? [store.getCategoryDescriptionsDetails.image]
      : [];
  });
});
</script>
            ...
            <VRow>
              ...
              <VCol md="6" cols="12">
                <VTextField
                  ...
                  :label="$t('common.section_name_en')"
                />
              </VCol>
              ...
              <VCol cols="12" class="d-flex flex-wrap gap-4">
                ...
                <VBtn
                  ...
                  variant="tonal"
                  type="reset"
                  :to="{ name: 'solar-energy-categories-description' }"
                >
                  {{ $t("common.cancel") }}
                </VBtn>
              </VCol>
