...
const onPageChange = (data) => {
  filterData.value.page = data;
  store.loadBanners(filterData);
};

const onper_pageChange = (data) => {
  filterData.value.per_page = data;
  store.loadBanners(filterData);
};
...
const deleteBannerFun = (id, status) => {
  let data = {
    id: id,
  };
  store.deleteBanner(data);
      ...
      <VCol cols="12">
        <VCard :title="$t('nav.banners')">
          ...
          <VCardText>
            <VRow>
              ...
              >
                ...
                <VBtn class="px-7" :to="{ name: 'banners-add' }">
                  {{ $t("common.add_banner") }}
                </VBtn>
              </VCol>
              <!-- 👉 Select Status -->
          ...
          <VTable class="text-no-wrap">
            ...
            <tbody>
              <tr
                v-for="(item, index) in store.getBanners"
                :key="index"
            ...
            <!-- 👉 table footer  -->
            <tfoot v-show="!store.getBanners.length">
              <tr>
          ...
          <VCardText
            v-if="store.getTotalBanners.currentPage"
          ...
          >
            ...
            <VPagination
              v-model="store.getTotalBanners.currentPage"
              size="small"
              :total-visible="5"
              :length="store.getTotalBanners.totalPages"
