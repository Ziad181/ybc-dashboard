...
export const useAdvisoryTeamsStore = defineStore({
  id: "advisory-teams",
  state: () => ({
    employees: [],
    totalEmployees: {
  ...
  actions: {
    async loadEmployees(data) {
      ...
      await this.$http
        .get(
          `/admin/advisory-teams?page=${data.value.page}&per_page=${data.value.per_page}&status=${
    ...
    async loadEmployeeDetails(id) {
      store.loadingStart();
      await this.$http
        .get(`/admin/advisory-teams/detail/${id}`)
        .then(response => {
          store.loadingFinish();
          if (response.status === 200) {
    ...
    async storEmployee(data) {
      ...
      return await this.$http
        .post(`/admin/advisory-teams/create`, data, {
          headers: {
    ...
    async updateEmployee(data) {
      ...
      return await this.$http
        .post(`/admin/advisory-teams/update/${data.id}`, data, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
    ...
    async employeeChangeStatus(data) {
      store.loadingStart();
      await this.$http
        .post(`/admin/advisory-teams/toggle/${data.id}`, data)
        .then(response => {
          store.loadingFinish();
          if (response.status === 200) {
            this.$notificationMessage({
    ...
    async deleteEmployee(data) {
      store.loadingStart();
      await this.$http
        .delete(`/admin/advisory-teams/delete/${data.id}`)
        .then(response => {
          store.loadingFinish();
          if (response.status === 200) {
            this.$notificationMessage({
